...
**ElevenLabs** reads it out, because the reason a lot of people cannot use a giving tool is
that it is a wall of financial text. The free tier is about ten minutes of audio a month, so
architecture had to do the work rather than budget: everything that never changes is
pre-rendered once at build time by `tools/narrate.mjs` into static mp3. A visitor who plays
the tour or the finding spends nothing. Only a brief about one specific filer is
synthesised live, cached in the Worker by a SHA-256 of the text, so the
second person to read the same organisation is free too. You can watch it happen: the
response carries `x-keepalive-audio: fresh` the first time and `cached` after.
...
than discovered, with a short list of defaults to fall back through. Free accounts also get `402 paid_plan_required` on library voices, so a voice id copied out of a
blog post is a dead end. The four in the list are ones I verified synthesise on this key.
...
- **Best Use of Google AI.** Gemini writes every brief under a JSON schema from a packet of
  that filer's own figures, with a rule that a missing field means not reported. It also writes
  the narration translations without touching a single figure.
...
- **Best Use of ElevenLabs.** The tour and the finding are pre-rendered at build time in four
  languages, so listening costs no credits and needs no key. A brief about one organisation is
  spoken live, then cached by a hash of the text.

The Snowflake path is written and in the repository as
[`data/snowflake.sql`](https://github.com/zkasuran/keepalive/blob/main/data/snowflake.sql),
and the live deploy tells you plainly that its credential is not bound: `/api/status`
returns `snowflake: false`. The finding tables you are reading were produced by the same SQL
run locally in DuckDB rather than in a warehouse. I would rather say that than claim a
category a judge cannot see working. If it changes before the deadline I will say so here in
an edit, with the timestamp.
